...
<head>
    ...
    <script language="JavaScript">
        $().ready(function () {
            ...
            $.ajax({
                url: "getProductData?exportId=" + $("#exportId").val(),
                success: function (d) {
                    if (d != null) {
                        var da = eval("("+d+")");
                        for (var i = 0; i < da.length; i++) {
                            var data = da[i];
                            addTRRecord('mRecordTable', data.exportProductId, nullChange(data.productNo), nullChange(data.cnumber), nullChange(data.grossWeight), nullChange(data.netWeight), nullChange(data.sizeLength), nullChange(data.sizeWidth),nullChange( data.sizeHeight), nullChange(data.exPrice), nullChange(data.tax));
                        }
                    }
                }
            });
...
<form name="icform" method="post">
    <input type="hidden" name="exportId" id="exportId" value="${export.exportId}"/>
    ...
    <div>
        <table class="commonTable" cellspacing="1">
            ...
            <tr>
                ...
                <td class="tableContent"><input type="text" name="destinationPort" value="${export.destinationPort}"/>
                </td>
            </tr>
            <tr>
                ...
                <td class="tableContent"><input type="text" name="priceCondition" value="${export.priceCondition}"/>
                </td>
            </tr>
            <tr>
