    ...
    <div>
        <table class="commonTable" cellspacing="1">
            <tr>
                ...
                <td class="tableContent">${c.customName }</td>
                <td class="columnTitle">打印版式：</td>
                <td class="tableContentAuto">
                    ${c.printStyle=='2'?"两款":"一款" }
                </td>
            </tr>
            <tr>
                <td class="columnTitle">合同号：</td>
                <td class="tableContent">${c.contractNo }</td>
                <td class="columnTitle">收购方：</td>
                <td class="tableContent">${c.offeror }</td>
            </tr>
            <tr>
                <td class="columnTitle">制单人：</td>
                <td class="tableContent">${c.inputBy }</td>
                <td class="columnTitle">审单人：</td>
                <td class="tableContent">${c.checkBy }</td>
            </tr>
            <tr>
                <td class="columnTitle">验货员：</td>
                <td class="tableContent">${c.inspector }</td>
                <td class="columnTitle">签单日期：</td>
                <td class="tableContent">
                    <fmt:formatDate value='${c.signingDate }' pattern='yyyy-MM-dd'/>
                </td>
            </tr>
            <tr>
                <td class="columnTitle">重要程度：</td>
                <td class="tableContentAuto">
                    ${c.importNum==3?"★★★":(c.importNum==2?"★★":"★") }
                </td>
                <td class="columnTitle">船期：</td>
                <td class="tableContent">
                    <fmt:formatDate value='${c.shipTime }' pattern='yyyy-MM-dd'/>
                </td>
            </tr>
            <tr>
                <td class="columnTitle">贸易条款：</td>
                <td class="tableContent">${c.tradeTerms }</td>
                <td class="columnTitle">交货期限：</td>
                <td class="tableContent">
                    <fmt:formatDate value='${c.deliveryPeriod }' pattern='yyyy-MM-dd'/>
                </td>
            </tr>
            <tr>
...
<div class="eXtremeTable">
    <table id="ec_table" class="tableRegion" width="98%">
        ...
        <c:forEach items="${c.contractProducts}" var="cp" varStatus="status">
            <tr bgcolor="#c3f3c3" height="30" class="odd" onmouseover="this.className='highlight'"
